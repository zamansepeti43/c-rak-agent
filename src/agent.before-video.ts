...
function detectFastTask(task: string): FastTask | null {
  ...
  const createRequested =
    /\b(oluştur|olustur|yarat)\b/i.test(text);

  const deleteRequested =
    /\b(sil|kaldır|kaldir)\b/i.test(text);

  const readRequested =
    /\b(oku|içeriğini söyle|icerigini soyle|içeriğini göster|icerigini goster)\b/i.test(text);

  const codingWords =
    /\b(düzelt|duzelt|refactor|geliştir|gelistir|hata|bug|build|lint|test et|analiz et|incele|değiştir|degistir)\b/i.test(text);
...
Sen �irak'sin.
Kullaniciyla T�rk�e, kisa ve dogal konus.
Kod g�revi verilmediyse dosya tarama, ara� kullanma veya analiz yapma.
