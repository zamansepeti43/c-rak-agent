...
function detectFastTask(task: string): FastTask | null {
  ...
  const createRequested =
    /\b(olustur|yarat)\b/i.test(text);

  const deleteRequested =
    /\b(sil|kaldir)\b/i.test(text);

  const readRequested =
    /\b(oku|icerigini\s+soyle|icerigini\s+goster)\b/i.test(text);

  const codingWords =
    /\b(duzelt|degistir|gelistir|refactor|hata|bug|build|lint|test\s+et|analiz\s+et|incele|degistir)\b/i.test(text);
...
Kullanıcı belirli bir dosyayı okuyup açıklamanı istiyor.
İlk adımda SADECE dosyayı oku.
Dosya içeriğini aldıktan sonra tekrar araç çağırma.
Dosya okunduktan sonra agent bu görevi doğrudan
hızlı açıklama aşamasına geçirir ve görev biter.
...
Kullanıcı basit, tek dosyalı bir görev verdi.
Gereksiz proje analizi yapma.
Sadece gerekli aracı seç.
...
Dosya okuma işlemi başarıyla tamamlandı.
Artık araç kullanma.
Kullanıcının istediği açıklamayı doğrudan ver.
...
Dosyanın ne yaptığını en fazla 5 kısa maddeyle açıkla.
    ...
    switch (
      ...
      case "video_producer": {
        const videoWorkspace =
          String.raw`C:\Users\Quantum\Desktop\VIDEO-SISTEMI`;

        const result =
          await runCommand(
            videoWorkspace,
            `python render_story.py ${JSON.stringify(action.prompt)}`
          );

        return JSON.stringify({
          ok: result.code === 0,
          output: compact(result.stdout, 6000),
          error: compact(result.stderr, 3000)
        });
      }
    }
  } catch (error) {
...
async function runAgent(
  ...
  const normalizedTask =
    normalizeCommandText(task);

  const videoRequested =
    /\b(video|videoyu|video\s+uret|video\s+olu[s?]tur|video\s+hazirla|cocuk\s+hikayesi|cocuk\s+hikayesi\s+uret|cocuk\s+hikayesi\s+olu[s?]tur|hikaye\s+uret|hikaye\s+olu[s?]tur|masal\s+uret|masal\s+olu[s?]tur|animasyon\s+uret|animasyon\s+olu[s?]tur)\b/i.test(
      normalizedTask
    );

  if (videoRequested) {
    console.log("\nVideo üretici çalıştırılıyor...");

    const videoAction: ToolAction = {
      type: "tool",
      tool: "video_producer",
      prompt: task
    };
    ...
    const videoParsed =
      parseResult(videoResult);

    console.log(
      `Sonuç: ${compact(videoResult, 2000)}`
    );
   ...
   * aramasi yapma. Kod g�revi degilse dogrudan hizli modele sor.
   */
  const chatTask =
    !fastTask &&
    !/\b(olustur|olustur|sil|kaldir|kaldir|oku|incele|analiz|d�zelt|duzelt|degistir|degistir|gelistir|gelistir|refactor|bug|hata|test et|build|lint|dosya|kod)\b/i.test(task);
...
Sen Cirak'sin.
Kullaniciyla Turkce, kisa ve dogal konus.
Kod gorevi verilmediyse dosya tarama, arama veya analiz yapma.
...
`);
  ...
  /*
   * HIZLI OKUMA:
   * Dosyayi oku ve a�ikla g�revlerinde normal 16 adimlik
   * agent d�ng�s�ne girme. Dogrudan dosyayi oku,
   * ardindan hizli model ile a�ikla.
   */
  if (
    fastTask &&
    /\b(oku|okuyup|i�erigini s�yle|icerigini soyle|i�erigini g�ster|icerigini goster)\b/i.test(task)
  ) {
    ...
    console.log(
      `Sonu�: ${compact(directResult, 1000)}`
    );
...
`;
    ...
    /*
     * Hedefli "dosyayı oku ve açıkla" görevlerinde
     * workspace taraması yapma ve read_file işlemini
     * tekrar ettirme. Sadece açıklama için ikinci,
     * kısa bir LLM çağrısı yap.
     */
    if (
      parsed.ok &&
      fastTask?.kind === "read" &&
